...
## 서버 저장소 (persist-server.js)

대기열·실적·감사 로그 등 대시보드의 모든 저장 데이터를 중앙 JSON 파일에
보관하는 참조 서버입니다. 브라우저를 바꿔도 같은 상태로 시작합니다.

```bash
npm run persist                  # http://localhost:8126 (--port·--file 로 변경)
```

대시보드 → 데이터 소스 설정 → **서버 저장소**에 주소 입력(관리자).
적용 시 현재 데이터를 일괄 업로드하고, 이후 변경은 키별 500ms 디바운스로
write-through 됩니다. 다음 접속부터는 서버 스냅샷을 선주입한 뒤 앱이 뜹니다.
서버가 죽어 있으면 3초 대기 후 로컬 데이터로 그대로 시작합니다(오프라인 내성).
참조 구현이라 인증이 없습니다 — 사내망 밖에서는 리버스 프록시로 보호하세요.
